...
const char MQTT_PUB_INTELLI_DATA[] = "$aws/rules/intelli_datadb/fivesec";
const char MQTT_PUB_INTELLI_DEVICES[] = "$aws/rules/intelli_devicesdb/onesec";
...
void sendData(void) {
    ...
    DynamicJsonDocument jsonBuffer2(JSON_OBJECT_SIZE(3) + 100);

    JsonObject sec5data = jsonBuffer.to<JsonObject>();
    ...
    if (NodeMCU.available() > 0) {
        while (NodeMCU.available() && i < 11) {
            MAIN_DATA[i++] = NodeMCU.read();
        }
        MAIN_DATA[i++] = '\0';
    }
